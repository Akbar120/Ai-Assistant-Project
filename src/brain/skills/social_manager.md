...
## Tool Access
- `instagram_dm_sender`: Use for sending direct messages to users on Instagram or Twitter.
...
## Decision Logic
1. **DMs:** If the user mentions "sending", "dm", "message", or "puch/bata" to a specific person, trigger `instagram_dm_sender`.
...
If a user asks to "Research X and then DM it to Y", spawn a `Research_Agent` first using `manage_agent`, then take the output and trigger `instagram_dm_sender`.
